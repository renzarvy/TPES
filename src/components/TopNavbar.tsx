...
export const TopNavbar: React.FC = () => {
  ...
  return (
    <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-xs print:hidden">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2 sm:py-2.5 flex flex-wrap md:flex-nowrap items-center justify-between gap-2 sm:gap-3">
        {/* Left: Section Breadcrumb & Active Page Badge */}
        <nav aria-label="Breadcrumb" className="flex items-center space-x-2 sm:space-x-3">
          <div className="flex items-center text-xs font-semibold text-slate-500">
            <span className="text-slate-900 font-bold">Portal</span>
            <ChevronRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 mx-0.5 sm:mx-1 text-slate-400" aria-hidden="true" />
            <span className="text-blue-700 font-extrabold capitalize truncate max-w-[100px] sm:max-w-none">{currentNavItem.name}</span>
          </div>

          <div className="h-3.5 w-[1px] bg-slate-200 hidden sm:block" aria-hidden="true" />
        ...
        {/* Center: Navigation Tabs Bar (Desktop) */}
        <nav aria-label="Portal section navigation" className="hidden md:flex items-center overflow-x-auto py-0.5 scrollbar-none gap-1 sm:gap-2">
          {filteredNavItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all whitespace-nowrap flex-shrink-0 ${
                  isActive
                    ? 'bg-gradient-to-r from-blue-900 to-slate-900 text-white shadow-sm ring-1 ring-blue-700'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100/80 border border-transparent'
                }`}
              >
                <Icon className={`w-3.5 h-3.5 mr-1.5 ${isActive ? 'text-amber-400' : 'text-slate-500'}`} aria-hidden="true" />
                <span>{item.name}</span>
                {isActive && (
                  <span className="ml-1.5 w-1.5 h-1.5 rounded-full bg-amber-400 shadow-[0_0_6px_#f59e0b]" aria-hidden="true" />
                )}
              </Link>
            );
          })}
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-2 ml-auto">
          ...
          <ServiceDiagnosticsModal
            triggerButtonClassName="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 transition cursor-pointer"
          />
